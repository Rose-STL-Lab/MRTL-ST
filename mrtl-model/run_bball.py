...
if args.type == 'multi':
    results['dims'] = [[[4, 5], [6, 6]], [[8, 10], [6, 6]], [[8, 10], [12, 12]],
...
if args.type == 'multi' or args.type == 'fixed':
  ...
  # prev_f = f
    ...
    plot.latent_factor_heatmap(prev_model_dict['C'],
                               ...
                               draw_court=False,
                               fp_fig=fp_fig)
    for i in range(len(player_style)):
        player_a, style_id = player_style[i]
        ...
        playstyle_heatmap = utils.reorder_weighted_heatmaps(prev_model_dict['F'][
                                                        ...
                                                        prev_model_dict['B'])
        fp_fig = os.path.join(fig_dir,
                          "full_{0},{1}_B_heatmap_style_{2}.png".format(b_str, c_str, i+1))
        ...
        player_style_heatmap = utils.reorder_weighted_heatmaps(player_style_values,
                                                        prev_model_dict['B'])
        fp_fig = os.path.join(fig_dir,
                          "full_{0},{1}_B_heatmap_player_style_{2}.png".format(b_str, c_str, i+1))
        ...
        playstyle_heatmap = utils.reorder_weighted_heatmaps(prev_model_dict['F'][
                                                        ...
                                                        prev_model_dict['C'])
        fp_fig = os.path.join(fig_dir,
                          "full_{0},{1}_C_heatmap_style_{2}.png".format(b_str, c_str, i+1))
        plot.latent_factor_heatmap(playstyle_heatmap, cmap='RdBu_r', draw_court=False,
                               fp_fig=fp_fig)
        player_style_values = torch.mul(prev_model_dict['A'][player_a],
                                        prev_model_dict['F'][style_id])
        player_style_heatmap = utils.reorder_weighted_heatmaps(player_style_values,
                                                        prev_model_dict['C'])
        fp_fig = os.path.join(fig_dir,
                          "full_{0},{1}_C_heatmap_player_style_{2}.png".format(b_str, c_str, i+1))
        plot.latent_factor_heatmap(player_style_heatmap, cmap='RdBu_r', draw_court=False,
                               fp_fig=fp_fig)
...
for i in range(len(player_style)):
    ...
    playstyle_heatmap = utils.reorder_weighted_heatmaps(multi.best_model_dict['F'][
                                                        ...
                                                        multi.best_model_dict['B'])
    fp_fig = os.path.join(fig_dir,
                     "low_{0},{1}_B_heatmap_style_{2}.png".format(b_str, c_str, i+1))
    plot.latent_factor_heatmap(playstyle_heatmap, cmap='RdBu_r', draw_court=True,
                          fp_fig=fp_fig)
    ...
    player_style_heatmap = utils.reorder_weighted_heatmaps(player_style_values,
                                                        multi.best_model_dict['B'])
    fp_fig = os.path.join(fig_dir,
                     "low_{0},{1}_B_heatmap_player_style_{2}.png".format(b_str, c_str, i+1))
    plot.latent_factor_heatmap(player_style_heatmap, cmap='RdBu_r', draw_court=True,
                           fp_fig=fp_fig)
    player_heatmap = utils.reorder_weighted_heatmaps(multi.best_model_dict['A'][
                                                    player_a],
    ...
    playstyle_heatmap = utils.reorder_weighted_heatmaps(multi.best_model_dict['F'][
                                                        ...
                                                        multi.best_model_dict['C'])
    fp_fig = os.path.join(fig_dir,
                     "low_{0},{1}_C_heatmap_style_{2}.png".format(b_str, c_str, i+1))
    plot.latent_factor_heatmap(playstyle_heatmap, cmap='RdBu_r', draw_court=False,
                           fp_fig=fp_fig)
    player_style_values = torch.mul(multi.best_model_dict['A'][player_a],
                                        multi.best_model_dict['F'][style_id])
    player_style_heatmap = utils.reorder_weighted_heatmaps(player_style_values,
                                                        multi.best_model_dict['C'])
    fp_fig = os.path.join(fig_dir,
                     "low_{0},{1}_C_heatmap_player_style_{2}.png".format(b_str, c_str, i+1))
    plot.latent_factor_heatmap(player_style_heatmap, cmap='RdBu_r', draw_court=False,
                           fp_fig=fp_fig)

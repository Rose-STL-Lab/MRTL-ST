...
for ((i = 1; i <= num_trials; i++)); do
  ...
  --low-lr "${rand_low_lr}" --low-reg "${rand_low_reg}" --delta "${delta}"
  echo "Trial ${i}: Multi"
  ...
  --batch-size "${batch_size}" --sigma "${sigma}" --K "${K}" --step-size "${step_size}" --gamma "${gamma}" \
  --full-lr "${fixed_full_lr}" --full-reg "${fixed_full_reg}" --low-lr "${fixed_low_lr}" --low-reg "${fixed_low_reg}" --delta "${delta}"
done

...
# Adding freeze support. Only applicable for Windows systems.
if __name__ == '__main__':
    ...
    # print( f"Bh_pos: max= {df.loc[:, 'bh_x':'bh_y'].max()}, min = {df.loc[:, 'bh_x':'bh_y'].min()}" ) print(
    # f"Def_pos_x: max= {df.filter(like='trunc_x').max(axis=0)}, min = {df.filter(like='trunc_x')[df.filter(
    # like='trunc_x') != -100.].min(axis=0)}" ) print( f"Def_pos_y: max= {df.filter(like='trunc_y').max(axis=0)},
    # min = {df.filter(like='trunc_y')[df.filter(like='trunc_y') != -100.].min(axis=0)}" ) print( f"At least one def for
    # every row: {(df.filter(like='trunc') != -100.).any(axis=1).sum() == df.shape[0]}" )

...
class BasketballMulti:
    ...
    def init_params(self, **kwargs):
        ...
        if 'sigma' in self.params:
            ...
            self.K_B = utils.create_kernel([self.dims[0][0]//7, self.dims[0][1]],
                                           self.params['sigma'], self.device)
            self.K_C = utils.create_kernel([self.dims[1][0]//7, self.dims[1][1]], self.params['sigma'],
                                           self.device)
...
#         print(counts)
    ...
    def train_and_evaluate(self, save_dir=None):
        ...
        while epochs < config.max_epochs:
            ...
            if type(self.model).__name__.startswith('Full'):
                # # Number of epochs = 5
                # if epochs >= 5:
...
#                     reg = self.params['reg_coeff'] #* (utils.l2_regularizer(self.model, self.device))
            ...
            # Constrain weights
            if type(self.model).__name__.startswith(
